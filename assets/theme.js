...
var ProductCard = class extends HTMLElement {
  ...
  async _getMatchingVariant(target) {
    $('.color-swatch').on('click', function(){
      var var__id = $(this).attr('data-id');
              $(this).closest("product-card").find('.custom__product_id').val(var__id);
        });
    ...
    if (this.hasAttribute("show-play-button") && !this.shadowRoot) {
      this.attachShadow({ mode: "open" }).appendChild(document.createRange().createContextualFragment(`
        ...
        <svg part="play-button" fill="none" width="48" height="48" viewBox="0 0 48 48">
          ...
          <path d="M18.578 32.629a.375.375 0 0 1-.578-.316V15.687c0-.297.328-.476.578-.316l12.931 8.314c.23.147.23.483 0 .63L18.578 32.63Z" fill="#111111"/>
          <path d="M24 .5C36.979.5 47.5 11.021 47.5 24S36.979 47.5 24 47.5.5 36.979.5 24 11.021.5 24 .5Z" stroke="#111111" stroke-opacity=".12"/>
        </svg>
      `));
    }
  }
